...
if (!$cycle) {
    ...
    $pdo->exec("INSERT INTO leaderboard_cycles (week_start, week_end, status) VALUES (DATE_SUB(CURDATE(), INTERVAL WEEKDAY(CURDATE()) DAY), DATE_ADD(DATE_SUB(CURDATE(), INTERVAL WEEKDAY(CURDATE()) DAY), INTERVAL 6 DAY), 'active')");
    echo "Done.\n"; exit(0);
}
$weekEnd = $cycle['week_end']; $now = date('Y-m-d');
if ($now <= $weekEnd) { echo "Still active until {$weekEnd}. No action.\n"; exit(0); }
$weekStart = $cycle['week_start']; $prizePerPerson = (float) $cycle['prize_per_person'];
echo "Paying out cycle {$cycle['id']} ({$weekStart} to {$weekEnd})...\n";
$top10 = $pdo->prepare("SELECT u.id, COUNT(r.id) AS referral_count FROM users u JOIN referrals r ON r.referrer_id = u.id AND r.bonus_paid = 1 WHERE u.role = 'earner' AND r.created_at >= :ws AND r.created_at < DATE_ADD(:we, INTERVAL 1 DAY) GROUP BY u.id ORDER BY referral_count DESC LIMIT 10");
...
try {
    ...
    // --- Seed new cycle with random referral data so leaderboard is not empty ---
    try {
        ...
        if ($cycleData) {
            $seedEarners = $pdo->query("SELECT id FROM users WHERE role = 'earner' AND status = 'active' ORDER BY RAND() LIMIT 20")->fetchAll(PDO::FETCH_COLUMN);
            $prevStmt = $pdo->query("SELECT user_id FROM leaderboard_payouts WHERE cycle_id = (SELECT id FROM leaderboard_cycles WHERE status = 'completed' ORDER BY closed_at DESC LIMIT 1) ORDER BY rank_position ASC LIMIT 5");
